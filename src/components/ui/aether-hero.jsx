...
/* Default fragment shader with numeric safety so it runs indefinitely */
...
// Cleanup
/* Shader canvas (background) */ /* Overlay gradient for readability */ /* Content layer */
...
vec3 scene(vec2 uv) {
  ...
  vec2 p = vec2(atan(uv.x, uv.y) * 2.0 / 6.28318, zoom);
  for (float i = 0.0; i < 3.0; i++) {
    int k = int(mod(i, 3.0));
    col[k] += pattern(p + i * 6.0 / MN);
  }
  return col;
}
void main() {
